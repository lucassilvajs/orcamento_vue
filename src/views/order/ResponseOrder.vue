...
<div class="mt-5">
  ...
  <b-row class="invoice-vue" v-if="order">
    <b-colxx xxs="12" lg="6" class="mb-5">
      <b-card v-if="response == 'approved' && order.status != 'reproved'" class="mb-4">
        <b-card-header>
      ...
      <b-card v-if="!feedbackSend">
        <b-card-header>
            <div v-if="response != 'approved' || order.status == 'reproved'">
      ...
      <b-card v-else>
        ...
        <div class="row">
          <div class="mb-3 col col-6">
            <span v-for="(o, orderIndex) in order.orders[0].attributes.info" :key="orderIndex" class="d-block mb-1"><b>{{orderIndex == 'name' ? 'Nome' : orderIndex }}</b>: {{o}} </span>
          </div>
          <div class="mb-3 col col-6">
            <div v-for="(o, orderIndex) in order.orders[0].attributes.lens" :key="orderIndex">
              <b>{{o.type}}: </b><br >{{o.name}} - {{o.price | numeroPreco}}
            </div>
          </div>
        </div>
      </b-card>
