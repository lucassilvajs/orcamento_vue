...
<div>
    <b-row class="app-row">
        <b-colxx xxs="12" class="chat-app">
            ...
            <conversation-detail :logged="this.$route.path.split('/')[1]" v-if="isLoadContacts && isLoadConversations && otherUser!=null && conversationMessages" key="conversation" :current-user='currentUser' :other-user='otherUser' :messages='conversationMessages' />
    ...
    methods: {
          ...
          if(this.message.attachment && type == 'image') {
            new Compressor(this.message.attachment, {
              ...
              success: async (result) => {
                ...
                message.type = 'image';
                fireApp.database().ref(`/sac/${sac}/`).push(...message)
              }
            })
        ...
        async getSac(){
          ...
          if(response.data.status != 'success') {
            ...
            this.sac = response.data.data.sac[0];
          }
        }
    },
    mounted() {
