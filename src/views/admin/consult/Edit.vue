    ...
    <b-row>
        <b-colxx xxs="12">
            <b-card class="mb-4" title="Consultor">
                <b-row>
                    <b-colxx md="4" lg="3">
                        <b-form-group label="Nome" class="has-float-label mb-4">
                            <b-form-input :disabled="true" v-model="consult.consult.name" type="text" placeholder="Nome" />
                        </b-form-group>
                    </b-colxx>
                    <b-colxx md="4" lg="3">
                        <b-form-group label="E-mail" class="has-float-label mb-4">
                            <b-form-input :disabled="!consult.consult.email" v-model="consult.consult.email" type="text" placeholder="E-mail" />
                        </b-form-group>
                    </b-colxx>
                    <b-colxx md="4" lg="3">
                ...
                <b-row>
                  <b-colxx>
                    <button class="btn btn-success float-right" @click="editConsult">Editar consultor</button>
                  </b-colxx>
                </b-row>
            </b-card>
        </b-colxx>
        <b-colxx xxs="12">
            <b-card class="mb-4" title="Empresas">
                <table class="table table-striped table-hover">
                  ...
                  <tbody v-if="consult.companies.length">
                    <tr v-for="(company, index) in consult.companies" :key="index">
                      <td>{{company.name}}</td>
                      <td>{{company.create | date}}</td>
                      <td v-if="company.last">{{company.last | date}}</td>
                      <td v-else> - </td>
                      <td>{{company.price | numeroPreco}}</td>
                    </tr>
                  </tbody>
                  <tbody v-else>
                    <tr v-for="comp in 20" :key="comp">
                      <td colspan="5" class="demo"></td>
                    </tr>
                  </tbody>
                </table>

                <b-pagination v-if="consult.total > 20"
                  size="sm"
                  align="center"
                  :total-rows="consult.total"
                  :per-page="20"
                  @change="pageCompany"
                />
            </b-card>
...
export default {
    ...
    methods: {
        ...
        async pageCompany(page){
          ...
          const response = await api.get(`/admin/consult/${id}`, {
            params
          });
        ...
        async editConsult() {
          const response = await api.put('admin/consult', {...this.consult.consult, image: this.image});
