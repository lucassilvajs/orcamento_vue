            ...
            <div v-for="(message, index) in messages" :key="`message${index}`">
              ...
              <b-card style="max-width:500px;"
                ...
                :class="{
                  ...
                  'float-left': message.sender == 'admin',
                  'float-right bg-green': message.sender == 'app'
                }"
              >
              <b-card-body>

                <a download v-if="message.file && message.file.value.length < 100" :href="baseURL + message.file.value">
...
export default {
  ...
  methods: {
    async getSac(sac){
      ...
      const response = await api.get(`sac/${sac}`);
    ...
    async getSacChat(sac){
      sac = sac ? sac : this.$route.params.id;
      const response = await api.get(`sac/chat/${sac}`);
    ...
    async sendMessage(){
      if(this.message.trim().length > 0 || this.file) {
        this.messages.push({
          sender: 'app', message: this.message, file: this.file, time: new Date()
        });
        ...
        const response = api.post(`sac/chat`, {
